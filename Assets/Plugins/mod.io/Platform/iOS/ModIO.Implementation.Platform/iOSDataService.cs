#if UNITY_IOS || (MODIO_COMPILE_ALL && UNITY_EDITOR)
...
{
    /// <summary>iOS implementation of the various data services.</summary>
    internal class iOSDataService : IUserDataService, IPersistentDataService, ITempDataService
    {
...
#region Initialization
                                                            ...
                                                            long gameId, BuildSettings settings)
        {
            ...
            this.rootDir = $"{iOSDataService.GlobalRootDirectory}/{gameId.ToString("00000")}/{userProfileIdentifier}";
            ...
            if(result.Succeeded())
            {
                Logger.Log(LogLevel.Verbose,
                           "Initialized iOSDataService for User Data: " + this.rootDir);
            }
            else
            {
                Logger.Log(LogLevel.Error, "Failed to initialize iOSDataService for User Data. "
                                                                  ...
                                                                  BuildSettings settings)
        {
            ...
            if(settings.extData != null
               && settings.extData.Count > iOSDataService.BSIndexPDSRootOverride)
            {
                Logger.Log(LogLevel.Verbose, "Persistent Root Directory loaded from extData");
                desiredRootDir = settings.extData[iOSDataService.BSIndexPDSRootOverride];
            }
            else
            {
                ResultAnd<byte[]> settingsRead =
                    await SystemIOWrapper.ReadFileAsync(iOSDataLayout.GlobalSettingsFilePath);
                result = settingsRead.result;

                iOSDataLayout.GlobalSettingsFile gsData;
                if(result.Succeeded()
                ...
                {
                    desiredRootDir = iOSDataLayout.DefaultPDSDirectory;

                    gsData = new iOSDataLayout.GlobalSettingsFile() {
                    ...
                    // ignore the result
                    await SystemIOWrapper.WriteFileAsync(iOSDataLayout.GlobalSettingsFilePath,
                                                         fileData);
                    Logger.Log(LogLevel.Verbose,
                ...
                {
                    string message =
                        $"Unable to initialize the persistent data service. globalsettings.json could not be parsed to load in the root data directory. FilePath: {iOSDataLayout.GlobalSettingsFilePath} - Result: [{result.code.ToString()}]";
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized iOSDataService for Persistent Data: " + rootDir);
        ...
        async Task<Result> ITempDataService.InitializeAsync(long gameId, BuildSettings settings)
        {
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized iOSDataService for Temp Data: " + rootDir);
