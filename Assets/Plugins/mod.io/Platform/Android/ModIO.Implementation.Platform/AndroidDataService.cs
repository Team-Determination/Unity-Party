#if UNITY_ANDROID || (MODIO_COMPILE_ALL && UNITY_EDITOR)
...
{
    /// <summary>Windows implementation of the various data services.</summary>
    internal class AndroidDataService : IUserDataService, IPersistentDataService, ITempDataService
    {
...
#region Initialization
                                                            ...
                                                            long gameId, BuildSettings settings)
        {
            ...
            this.rootDir = $"{AndroidDataService.GlobalRootDirectory}/{gameId.ToString("00000")}/{userProfileIdentifier}";
            ...
            if(result.Succeeded())
            {
                Logger.Log(LogLevel.Verbose,
                           "Initialized WindowsDataService for User Data: " + this.rootDir);
            }
            else
            {
                Logger.Log(LogLevel.Error, "Failed to initialize WindowsDataService for User Data. "
                                                                  ...
                                                                  BuildSettings settings)
        {
            ...
            if(settings.extData != null
               && settings.extData.Count > AndroidDataService.BSIndexPDSRootOverride)
            {
                Logger.Log(LogLevel.Verbose, "Persistent Root Directory loaded from extData");
                desiredRootDir = settings.extData[AndroidDataService.BSIndexPDSRootOverride];
            }
            else
            {
                ResultAnd<byte[]> settingsRead =
                    await SystemIOWrapper.ReadFileAsync(AndroidDataLayout.GlobalSettingsFilePath);
                result = settingsRead.result;

                AndroidDataLayout.GlobalSettingsFile gsData;
                if(result.Succeeded()
                ...
                {
                    desiredRootDir = AndroidDataLayout.DefaultPDSDirectory;

                    gsData = new AndroidDataLayout.GlobalSettingsFile() {
                    ...
                    // ignore the result
                    await SystemIOWrapper.WriteFileAsync(AndroidDataLayout.GlobalSettingsFilePath,
                                                         fileData);
                    Logger.Log(LogLevel.Verbose,
                ...
                {
                    string message =
                        $"Unable to initialize the persistent data service. globalsettings.json could not be parsed to load in the root data directory. FilePath: {WindowsDataLayout.GlobalSettingsFilePath} - Result: [{result.code.ToString()}]";
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized WindowsDataService for Persistent Data: " + rootDir);
        ...
        async Task<Result> ITempDataService.InitializeAsync(long gameId, BuildSettings settings)
        {
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized WindowsDataService for Temp Data: " + rootDir);
