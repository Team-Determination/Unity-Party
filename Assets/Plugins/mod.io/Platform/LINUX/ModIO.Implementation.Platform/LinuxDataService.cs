#if UNITY_STANDALONE_LINUX || (MODIO_COMPILE_ALL && UNITY_EDITOR)
...
{
    /// <summary>Linux implementation of the various data services.</summary>
    internal class LinuxDataService : IUserDataService, IPersistentDataService, ITempDataService
    {
...
#region Initialization
                                                            ...
                                                            long gameId, BuildSettings settings)
        {
            ...
            this.rootDir = $"{LinuxDataService.GlobalRootDirectory}/{gameId.ToString("00000")}/{userProfileIdentifier}";
            ...
            if(result.Succeeded())
            {
                Logger.Log(LogLevel.Verbose,
                           "Initialized LinuxDataService for User Data: " + this.rootDir);
            }
            else
            {
                Logger.Log(LogLevel.Error, "Failed to initialize LinuxDataService for User Data. "
                                                                  ...
                                                                  BuildSettings settings)
        {
            ...
            if(settings.extData != null
               && settings.extData.Count > LinuxDataService.BSIndexPDSRootOverride)
            {
                Logger.Log(LogLevel.Verbose, "Persistent Root Directory loaded from extData");
                desiredRootDir = settings.extData[LinuxDataService.BSIndexPDSRootOverride];
            }
            else
            {
                ResultAnd<byte[]> settingsRead =
                    await SystemIOWrapper.ReadFileAsync(LinuxDataLayout.GlobalSettingsFilePath);
                result = settingsRead.result;

                LinuxDataLayout.GlobalSettingsFile gsData;
                if(result.Succeeded()
                ...
                {
                    desiredRootDir = LinuxDataLayout.DefaultPDSDirectory;

                    gsData = new LinuxDataLayout.GlobalSettingsFile() {
                    ...
                    // ignore the result
                    await SystemIOWrapper.WriteFileAsync(LinuxDataLayout.GlobalSettingsFilePath,
                                                         fileData);
                    Logger.Log(LogLevel.Verbose,
                ...
                {
                    string message =
                        $"Unable to initialize the persistent data service. globalsettings.json could not be parsed to load in the root data directory. FilePath: {LinuxDataLayout.GlobalSettingsFilePath} - Result: [{result.code.ToString()}]";
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized LinuxDataService for Persistent Data: " + rootDir);
        ...
        async Task<Result> ITempDataService.InitializeAsync(long gameId, BuildSettings settings)
        {
            ...
            Logger.Log(LogLevel.Verbose,
                       "Initialized LinuxDataService for Temp Data: " + rootDir);
