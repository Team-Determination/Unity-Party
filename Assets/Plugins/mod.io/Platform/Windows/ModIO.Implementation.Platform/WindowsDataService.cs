...
namespace ModIO.Implementation.Platform
{
    ...
    internal class WindowsDataService : IUserDataService, IPersistentDataService, ITempDataService
    {
...
#region Initialization
                                                            ...
                                                            long gameId, BuildSettings settings)
        {
            ...
            this.rootDir = $"{WindowsDataService.GlobalRootDirectory}/{gameId.ToString("00000")}/{userProfileIdentifier}";
                                                                  ...
                                                                  BuildSettings settings)
        {
            ...
            if(settings.extData != null
               && settings.extData.Count > WindowsDataService.BSIndexPDSRootOverride)
            {
                Logger.Log(LogLevel.Verbose, "Persistent Root Directory loaded from extData");
                desiredRootDir = settings.extData[WindowsDataService.BSIndexPDSRootOverride];
            }
            else
            {
                ResultAnd<byte[]> settingsRead =
                    await SystemIOWrapper.ReadFileAsync(WindowsDataLayout.GlobalSettingsFilePath);
                result = settingsRead.result;

                WindowsDataLayout.GlobalSettingsFile gsData;
                if(result.Succeeded()
                ...
                {
                    desiredRootDir = WindowsDataLayout.DefaultPDSDirectory;

                    gsData = new WindowsDataLayout.GlobalSettingsFile() {
                    ...
                    // ignore the result
                    await SystemIOWrapper.WriteFileAsync(WindowsDataLayout.GlobalSettingsFilePath,
                                                         fileData);
                    Logger.Log(LogLevel.Verbose,
